...
void main() {
  ...
  test("listApplications", () async {
    await database.customInsert(
      "INSERT INTO apps(package_name, name, version, banner, icon)"
  ...
  test("updateApp", () async {
    await database.customInsert(
      "INSERT INTO apps(package_name, name, version, banner, icon, hidden)"
  ...
  test("insertCategory", () async {
    await database.insertCategory(
      CategoriesCompanion.insert(name: "Test", order: 2),
    );
  ...
  test("deleteCategory", () async {
    ...
    final app = await database.customSelect("SELECT * FROM apps;").getSingle();
    expect(app.read<String>("package_name"), "io.sifft.pitchforklauncher");
    final appsCategories = await database
  ...
  test("updateCategory", () async {
    ...
    await database.updateCategory(
      categoryId,
      CategoriesCompanion(order: Value(5)),
    );
  ...
  test("insertAppsCategories", () async {
    ...
    expect(
      appCategory.read<String>("app_package_name"),
      "io.sifft.pitchforklauncher",
    );
  ...
  test("listCategoriesWithApps", () async {
    ...
    expect(
      categoriesWithApps[0].applications[0].packageName,
      "io.sifft.pitchforklauncher.2",
    );
    expect(categoriesWithApps[0].applications[0].name, "FLauncher 2");
    expect(
      categoriesWithApps[0].applications[1].packageName,
      "io.sifft.pitchforklauncher",
    );
  ...
  test("listCategoriesWithAllApps includes hidden apps", () async {
    ...
    expect(
      categoriesWithApps[0].applications[2].packageName,
      "io.sifft.pitchforklauncher.3",
    );
