...
void main() {
  ...
  testWidgets("mappings are displayed", (tester) async {
    ...
    when(buttonMappingService.mappings).thenReturn([
      ButtonMapping(190, "KEYCODE_BUTTON_3", "com.google.android.youtube.tv"),
    ]);
  ...
  testWidgets("falls back to the package name if the app is unknown", (
    tester,
  ) async {
    final buttonMappingService = MockButtonMappingService();
    final appsService = MockAppsService();
  ...
  testWidgets(
    "'Add mapping' captures a button then sets the mapping from the picked app",
    (tester) async {
      final buttonMappingService = MockButtonMappingService();
      final appsService = MockAppsService();
      when(buttonMappingService.mappings).thenReturn([]);
      ...
      when(buttonMappingService.captureNextButton()).thenAnswer(
        (_) => Stream.value({"keyCode": 191, "label": "KEYCODE_BUTTON_4"}),
      );
  ...
  testWidgets("'Cancel' in the capture dialog doesn't set a mapping", (
    tester,
  ) async {
    final buttonMappingService = MockButtonMappingService();
    final appsService = MockAppsService();
    ...
    MultiProvider(
      providers: [
        ChangeNotifierProvider<ButtonMappingService>.value(
          value: buttonMappingService,
        ),
        ChangeNotifierProvider<AppsService>.value(value: appsService),
      ],
      builder: (_, _) =>
          MaterialApp(home: Scaffold(body: ButtonMappingPanelPage())),
    ),
  );
  await tester.pumpAndSettle();
}
